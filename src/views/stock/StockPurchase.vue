<template>
  <div>
    ...
    <div class="option-button">
      ...
      <el-button type="primary" size="medium" @click.native="addBook"
        >新增
      </el-button>
      <el-button type="success" size="medium" @click.native="downloadFile"
        >模板下载
      </el-button>
      <el-upload
        class="form-upload-excel"
        ref="uploadExcel"
        name="bookList"
        action="#"
        accept=".xlsx, .xls"
    ...
    <div class="table-container">
      ...
      >
        ...
        <!-- <template v-for="item in tableItem">
          ...
          ></el-table-column>
          <el-table-column
            v-if="item.name === 'author' && item.isShow"
            key="author"
            prop="author"
            align="center"
            label="作者"
          ...
          ></el-table-column>
          <el-table-column
            v-if="item.name === 'press' && item.isShow"
            key="press"
            prop="press"
            align="center"
            label="出版社"
          ...
          ></el-table-column>
          <el-table-column
            v-if="item.name === 'isSell' && item.isShow"
            key="isSell"
            align="center"
            label="是否在售"
            width="100"
          >
            <template slot-scope="scope">
              <span v-if="scope.row.isSell == 1" class="status-success"
                >是</span
              >
          ...
          <el-table-column
            v-if="item.name === 'classify' && item.isShow"
            key="classify"
            align="center"
            label="分类"
            width="180"
          >
            <template slot-scope="scope">
              <show-tags
                :classify="scope.row.classify"
                :classifyMap="classifyMap"
              ></show-tags>
            </template>
          </el-table-column>
          <el-table-column
            v-if="item.name === 'title' && item.isShow"
            key="title"
            align="center"
            label="标题"
            width="180"
          ...
          <el-table-column
            v-if="item.name === 'description' && item.isShow"
            key="description"
            align="center"
            label="描述"
            width="180"
            ...
            <template slot-scope="scope">
              <span :title="scope.row.description">{{
                scope.row.description
                  ? scope.row.description.length > 8
          ...
          <el-table-column
            v-if="item.name === 'stock' && item.isShow"
            key="stock"
            prop="stock"
            align="center"
            label="库存"
          ...
          ></el-table-column>
          <el-table-column
            v-if="item.name === 'price' && item.isShow"
            key="price"
            align="center"
            label="价格"
            width="100"
          ...
          <el-table-column
            v-if="item.name === 'salePrice' && item.isShow"
            key="salePrice"
            align="center"
            label="折后价"
            width="100"
          ...
          <el-table-column
            v-if="item.name === 'imageUrl' && item.isShow"
            key="imageUrl"
            align="center"
            label="图片"
            width="100"
          >
            <template slot-scope="scope">
              <el-button
                ...
                type="text"
                size="mini"
                @click.native="showImgDialogFun(scope.row.imageUrl)"
                >查看图片
              </el-button>
              <span v-else>---</span>
            </template>
        ...
        <el-table-column fixed="right" align="center" label="操作" width="140">
          <template slot-scope="scope">
            ...
            <el-button
              type="text"
              size="mini"
              @click.native="editStockFun(scope.row)"
              >库存修改
            </el-button>
          </template>
        </el-table-column>
    ...
    >
      ...
      >
        ...
        >
          <el-input
            v-model.trim="editData.stockPrice"
            type="number"
            step="0.01"
            autocomplete="off"
          ></el-input>
    ...
    >
      ...
      >
        <el-form-item label="操作类型" label-width="80px">
          <el-radio-group
            ...
            @change="radioChange"
          >
            <el-radio :label="0" border>进货 </el-radio>
            <el-radio :label="1" border>删除 </el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="库存量" prop="changeStock" label-width="80px">
          <div class="in-stock" v-if="radioSwitch == 0">
            ...
            <el-input-number
              v-model.trim="editStock"
              size="small"
              :min="0"
              :step="1"
            ></el-input-number>
          </div>
          <div class="out-stock" v-else>
            <span>{{ editData.stock + editStock }}</span>
            <el-tooltip
              effect="dark"
              content="实际更新数量已提交后为准"
      ...
      <div slot="footer" class="dialog-footer">
        <el-button size="small" @click="editStockDialog = false"
          >取 消
        </el-button>
        <el-button
          type="primary"
          size="small"
          @click="submitEditStock('editStockForm')"
          >确 定
        </el-button>
      </div>
    </el-dialog>
    ...
    return {
      ...
      editDataValidate: {
        ...
        changeStock: [
          {
            required: true,
            validator: this.changeStockRegFun,
            trigger: "blur",
          },
        ],
  ...
  methods: {
    ...
    async deleteBook() {
      ...
      this.$confirm(`确定删除所选图书吗？`, "提示", {
        ...
        .then(async () => {
          try {
            let res = await bookApi.delBook(obj);
            if (res.errorCode === 200) {
              this.$message({
                message: "删除成功",
                type: "success",
